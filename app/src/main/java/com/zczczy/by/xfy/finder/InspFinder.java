 ...
 */
@EBean
public class InspFinder  extends  BaseFinder<Insp>  {
    ...
    public void pagingQuery(int pageIndex, int pageSize, Object... objects) {
        ...
        if(AndroidTool.isNetworkAvailable(connectivityManager)){
            BaseModelJson<PagerResult<Insp>> bmj =myRestClient.GetACheckList((objects[0]).toString(), pageIndex, pageSize, (objects[1]).toString());
    ...
    @Override
    @UiThread
    public void afterPagingQuery(BaseModelJson<PagerResult<Insp>> bmj) {
